...
class MSDeformableAttention(nn.Module):
    ...
    def forward(self,
        ...
        if reference_points.shape[-1] == 2:
            ...
            # reference_points [8, 480, None, 1,  5]
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        ref_points_detach = F.sigmoid(ref_points_unact) # ref_points_unact (-inf,inf)
        ...
        for i, layer in enumerate(self.layers):
            ...
            output = layer(output, ref_points_input, memory, memory_spatial_shapes, attn_mask, memory_mask, query_pos_embed)    #[bs,num_queries,embed_dim]
...
class RTDETRTransformerv2(nn.Module):
    ...
    def __init__(self,
        ...
        self.query_pos_head = MLP(5, 2 * hidden_dim, hidden_dim, 2) # 
        ...
        self.enc_bbox_head = MLP(hidden_dim, hidden_dim, 5, 3)
        ...
        self.dec_bbox_head = nn.ModuleList([
            MLP(hidden_dim, hidden_dim, 5, 3) for _ in range(num_layers)    # 
        ])
    ...
    def _generate_anchors(self,
        ...
        for lvl, (h, w) in enumerate(spatial_shapes):  
            ...
            with torch.random.fork_rng():
                ...
                theta = torch.rand_like(grid_xy[..., :1]) 
            lvl_anchors = torch.concat([grid_xy, wh,theta], dim=-1).reshape(-1, h * w, 5) #(1,h,w,4)[xywh\theta] -> (1,hw,5)
            anchors.append(lvl_anchors) 

        anchors = torch.concat(anchors, dim=1).to(device)   #(1,sum(hw),5)
    ...
    def _get_decoder_input(self,
        ...
        enc_outputs_logits :torch.Tensor = self.enc_score_head(output_memory)   # (bs,sum(hw),num_classes)
        ...
        if denoising_bbox_unact is not None:
            enc_topk_bbox_unact = torch.concat([denoising_bbox_unact, enc_topk_bbox_unact], dim=1)  # (bs,num_dn+topk,4)
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_coord, outputs_angle=None):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        if outputs_angle is not None:
            return [{'pred_logits': a, 'pred_boxes': b, 'pred_angles': c}
                    for a, b, c in zip(outputs_class, outputs_coord, outputs_angle)]
        else:
