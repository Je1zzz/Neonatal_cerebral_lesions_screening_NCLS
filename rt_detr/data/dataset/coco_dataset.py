...
class CocoDetection(torchvision.datasets.CocoDetection, DetDataset):
    ...
    def load_item(self, idx):
        ...
        if self.remap_mscoco_category:
            image, target = self.prepare(image, target, category2label=neonatal_cranium_category2label)     
        ...
        if 'boxes' in target:
            target['boxes'] = convert_to_tv_tensor(target['boxes'], key='boxes', spatial_size=image.size[::-1],box_format='xyxy')   # 此时的boxes并没有归一化
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image: Image.Image, target, **kwargs):
        ...
        # guard against no boxes via resizing, 转换为xywh -> xyxy
        boxes = torch.as_tensor(boxes, dtype=torch.float32).reshape(-1, 4)  
        boxes[:, 2:] += boxes[:, :2]
        boxes[:, 0::2].clamp_(min=0, max=w)
        boxes[:, 1::2].clamp_(min=0, max=h)

        angles = [obj['angle'] for obj in anno]
