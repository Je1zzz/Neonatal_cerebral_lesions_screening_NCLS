...
def convex_hull(points):
    ...
    lower = []
    for p in points:
        while len(lower) >= 2 and cross(lower[-2], lower[-1], p) <= 0:  # clockwise 达咩
            lower.pop()
        lower.append(p)
...
def GIoU_Rotated_Rectangle(box1,box2):
    ...
    GIoU_matrix = torch.zeros((N, M), dtype=torch.float64, device=box1.device, requires_grad=False)
    IoU_matrix = torch.zeros((N, M), dtype=torch.float64, device=box1.device, requires_grad=False)
    
    for i in range(N):
        for j in range(M):
            Box1, Box2 = box1[i], box2[j]
            if len(Box1) == 5:
            ...
            IoU = area_box1_intersection_box2/area_box1_union_box2
            GIoU = area_box1_intersection_box2/area_box1_union_box2 - (C-area_box1_union_box2)/C
            IoU_matrix[i, j] = IoU
            GIoU_matrix[i, j] = GIoU
    GIoU_matrix.requires_grad_(True)
    IoU_matrix.requires_grad_(True)
    return GIoU_matrix, IoU_matrix
