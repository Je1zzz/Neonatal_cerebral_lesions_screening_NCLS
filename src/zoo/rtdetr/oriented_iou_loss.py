...
def cal_iou(box1:torch.Tensor, box2:torch.Tensor):
    ...
    Args:
        ...
        box2 (torch.Tensor): (B, M, 5)
    Returns:
        iou (torch.Tensor): (B, N, M)
        corners1 (torch.Tensor): (B, N, 4, 2)
        corners2 (torch.Tensor): (B, M, 4, 2)
        U (torch.Tensor): (B, N, M) area1 + area2 - inter_area
    """
    corners1 = box2corners_th(box1)
    corners2 = box2corners_th(box2)
    inter_area, _ = oriented_box_intersection_2d(corners1, corners2)  # (B, N, M)
    area1 = box1[:, :, 2] * box1[:, :, 3]
    area2 = box2[:, :, 2] * box2[:, :, 3]
    u = area1.unsqueeze(2) + area2.unsqueeze(1) - inter_area
...
def cal_diou(box1:torch.Tensor, box2:torch.Tensor, enclosing_type:str="smallest"):
    ...
    """
    if box2.shape[1] == 0 or box1.shape[1] == 0:
        return 0.,[],[]
    iou, corners1, corners2, u = cal_iou(box1, box2)
    w, h = enclosing_box(corners1, corners2, enclosing_type)
    c2 = w*w + h*h      # (B, N)
    x_offset = box1[...,0].unsqueeze(2) - box2[..., 0].unsqueeze(1)
    y_offset = box1[...,1].unsqueeze(2) - box2[..., 1].unsqueeze(1)
    d2 = x_offset*x_offset + y_offset*y_offset
    diou = iou + d2/c2
    diou_loss = 1. - iou + d2/c2
    return diou_loss, diou, iou
...
def enclosing_box(corners1:torch.Tensor, corners2:torch.Tensor, enclosing_type:str="smallest"):
    if enclosing_type == "aligned":
        ...
        corners1_expanded = corners1.unsqueeze(2).expand(-1, -1, corners2.size(1), -1, -1)  # (bs, N, M, 4, 2)
        corners2_expanded = corners2.unsqueeze(1).expand(-1, corners1.size(1), -1, -1, -1)  # (bs, N, M, 4, 2)
        return smallest_bounding_box(torch.cat([corners1_expanded, corners2_expanded], dim=-2))
    else:
